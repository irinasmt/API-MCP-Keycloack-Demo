...
Write-Host "🚀 Starting Product API & MCP Server (Dynamic Registration)..." -ForegroundColor Green
...
# Start MCP server with dynamic registration
Write-Host "Starting MCP Server (Dynamic Registration) on port 3002..." -ForegroundColor Cyan
Start-Process pwsh -ArgumentList "-NoExit", "-Command", "cd mcp; bun install; bun run dev-dynamic" -WindowStyle Normal
...
if (Test-Path $mcpConfigPath) {
    ...
    # Check if product-mcp-dynamic already exists
    if (-not $config.servers.'product-mcp-dynamic') {
        $config.servers | Add-Member -MemberType NoteProperty -Name "product-mcp-dynamic" -Value @{
            url = "http://localhost:3002"
            type = "http"
        } -Force
        
        $config | ConvertTo-Json -Depth 10 | Set-Content $mcpConfigPath
        Write-Host "✅ Added product-mcp-dynamic to VS Code MCP configuration" -ForegroundColor Green
    } else {
        Write-Host "✅ product-mcp-dynamic already configured in VS Code" -ForegroundColor Green
    }
} else {
...
Write-Host "MCP (Dynamic): http://localhost:3002" -ForegroundColor White
Write-Host "`n💡 Remember to:" -ForegroundColor Yellow
Write-Host "  1. Start Keycloak: docker-compose up -d" -ForegroundColor White
Write-Host "  2. Configure Keycloak with DCR enabled (see Dynamic-Client-Registration-Flow.md)" -ForegroundColor White
Write-Host "  3. Update .env files if needed" -ForegroundColor White
Write-Host "  4. Reload VS Code to pick up MCP changes" -ForegroundColor White
Write-Host "`n🔄 Dynamic Client Registration:" -ForegroundColor Cyan
Write-Host "  - Client will register automatically with Keycloak" -ForegroundColor White
Write-Host "  - No pre-configured client_id needed" -ForegroundColor White
